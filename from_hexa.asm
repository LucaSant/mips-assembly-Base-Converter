...
loop:
	...
	sll $t6, $t6, 4			#$t6 (que será a saída) é deslocado 4 bits para a esquerda. Isso é feito para depois somar $t1 (0 a 15) em seu primeiro byte
...
loop_check:				#loop percorre a string hexadecimal para achar o caractere correspondente
	...
	beq $s2, $t0, valid 		#Se o caractere atual, dentro o input do usuário, for igual a um dos caracteres hexadecimais, então o caractere está válido 
...
valid:					#quando o caractere é válido, temos seu número correspondente dentro da string hexadecimal, o valor está em $t1.
...
end_loop:					#fim da conversão de hexa para decimal
	...
	jr $ra						#caso contrário, outra função a chamou para ser intermediária ou para fazer a validação da entrada

hexa_to_bin: 				
	
	jal hexa_to_dec		#chama para converter de hexa para decimal 
	move $t2, $t6			#como a saída de hexa_to_dec é $t6 e a entrada de dec_to_bin é $t2, precisamos mover 
	j dec_to_bin			#chama para converte de decimal para binario. dec_to_bin mandará para print_output_bin_hexa
